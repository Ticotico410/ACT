...
class PickPolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        self.trajectory = [
            ...
            {"t": 50, "xyz": box_xyz + np.array([0, 0, 0.2]), "quat": gripper_pick_quat.elements, "gripper": 1},     # approach the cube
            {"t": 200, "xyz": init_mocap_pose[:3], "quat": init_mocap_pose[3:], "gripper": 0},     # sleep
        ]
    ...
    for episode_idx in range(1):
        ...
        for step in range(episode_len):
            ...
            if step == episode_len - 1:
                print(f"ts={episode_len} qpos: {ts.observation['qpos']}")
                print(f"ts={episode_len} qpos shape: {ts.observation['qpos'].shape}")
            if onscreen_render:
