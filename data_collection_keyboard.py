...
# Keyboard Mocap Control
SPACE_KEYS = {32}
ENTER_KEYS = {13, 257}
ESC_KEYS = {27, 256}
GRIPPER_OPEN_KEYS = {69, 101}   # E: Open 
GRIPPER_CLOSE_KEYS = {82, 114}  # R: Close
MOVE_FORWARD_KEYS = {87, 119}   # W: Forward
MOVE_BACKWARD_KEYS = {83, 115}  # S: Backward
MOVE_LEFT_KEYS = {65, 97}       # A: Left
MOVE_RIGHT_KEYS = {68, 100}     # D: Right
MOVE_UP_KEYS = {75, 107}        # K: Up
MOVE_DOWN_KEYS = {76, 108}      # L: Down
MAX_GRIPPER_STEP = 0.0005       # Maximum gripper step size
MOCAP_MOVE_STEP = 0.01         # Keyboard mocap translation step
...
def main() -> None:
    ...
    parser.add_argument("--guide-cameras", type=str, default="angle,top,left_pillar,right_pillar", help="Fixed guide window")
    ...
    print("[info] Mocap keyboard control: W/S forward/backward, A/D left/right, K/L up/down")
    ...
    pending_mocap_delta = np.zeros(3, dtype=np.float64)

    def key_callback(keycode: int) -> None:
        nonlocal gripper_target, record_step_counter, pending_mocap_delta
        ...
        elif keycode in MOVE_FORWARD_KEYS:
            pending_mocap_delta[1] += MOCAP_MOVE_STEP
        elif keycode in MOVE_BACKWARD_KEYS:
            pending_mocap_delta[1] -= MOCAP_MOVE_STEP
        elif keycode in MOVE_LEFT_KEYS:
            pending_mocap_delta[0] -= MOCAP_MOVE_STEP
        elif keycode in MOVE_RIGHT_KEYS:
            pending_mocap_delta[0] += MOCAP_MOVE_STEP
        elif keycode in MOVE_UP_KEYS:
            pending_mocap_delta[2] += MOCAP_MOVE_STEP
        elif keycode in MOVE_DOWN_KEYS:
            pending_mocap_delta[2] -= MOCAP_MOVE_STEP
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        while viewer.is_running() and not flags["exit"]:
            ...
            if np.any(pending_mocap_delta):
                data.mocap_pos[mocap_id] = data.mocap_pos[mocap_id] + pending_mocap_delta
                pending_mocap_delta[:] = 0.0
