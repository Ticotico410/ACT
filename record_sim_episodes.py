...
def main(args):
    ...
    for episode_idx in range(num_episodes):
        ...
        ts = env.reset()
        episode = [ts]
        policy = policy_cls(inject_noise, physics=env._physics)
        # setup plotting
        if onscreen_render:
            ax = plt.subplot()
        ...
        for joint, ctrl in zip(joint_traj, gripper_ctrl_traj):
            ...
            gripper_ctrl_normalized = PUPPET_GRIPPER_POSITION_NORMALIZE_FN(ctrl[0])
            joint[7] = gripper_ctrl_normalized  # Replace gripper qpos with normalized control
        ...
        - qpos                  (8,)         'float64'  # 7 joints + 1 gripper
        - qvel                  (8,)         'float64'
